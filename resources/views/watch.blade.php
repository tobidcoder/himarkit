	...
	<body class="player-course-chapter">
		...
		<ul id="courseplaylist" class="display-none">
			...
			@foreach($courses->chapter as $chapter)
				...
				<li data-source="courseplaycontent{{ $chapter->id }}" data-playlist-name="{{ isset($chapter->chapter_name) }}" data-thumbnail-path="{{ url('images/course/'.$courses->preview_image) }}">
					<p class="minimalDarkCategoriesTitle"><span class="minimialDarkBold">Title: </span>{{ isset($chapter->chapter_name) }}</p>
					<p class="minimalDarkCategoriesType"><span class="minimialDarkBold">Category: </span>{{ isset($courses->category->title) }}</p>
